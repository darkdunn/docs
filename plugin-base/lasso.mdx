...
套索（Lasso）是笔记/文档中常用的“圈选”交互：用户用笔在页面上圈出一个区域后，系统会基于该区域计算选中的元素集合，并显示套索工具条（Lasso Toolbar）。
插件可以在套索工具条上展示自己注册的按钮；用户点击按钮后进入插件界面，插件即可基于当前套索上下文读取与修改相关数据。

## 交互流程

1. 用户在页面上完成套索圈选
2. 系统显示套索工具条（可展示插件注册按钮）
3. 用户点击插件按钮进入插件界面
...
套索选中的“元素列表”通过 [`PluginCommAPI.getLassoElements()`](/api-reference/supernote-plugin/plugin-comm-api/get-lasso-data) 获取，返回值是 [`Element[]`](/api-reference/supernote-plugin/types/trail)。

- 通过 `Element.type` 区分元素类别（常量见 [`ElementType`](/api-reference/supernote-plugin/types/trail#elementtype)）
- 对于大体量点数据（例如角度点、轮廓点等），`Element` 字段会以访问器形式提供（[`ElementDataAccessor`](/api-reference/supernote-plugin/types/trail-data-accessor)），避免一次性把大规模点集传到 RN 侧造成内存压力

## 常用接口

| 目标 | 接口 | 说明 |
| --- | --- | --- |
| 获取套索框位置 | [`getLassoRect()`](/api-reference/supernote-plugin/plugin-comm-api/get-lasso-rect) | 返回套索框矩形（`Rect`） |
| 更新套索框位置 | [`updateLassoRect(rect)`](/api-reference/supernote-plugin/plugin-comm-api/update-lasso-rect) | 更新套索框矩形。当前仅支持等比例缩放 |
| 获取套索元素列表 | [`getLassoElements()`](/api-reference/supernote-plugin/plugin-comm-api/get-lasso-data) | 返回 `Element[]`（成功时会补齐访问器字段） |
| 控制套索框状态 | [`setLassoBoxState(state)`](/api-reference/supernote-plugin/plugin-comm-api/set-lasso-box-state) | `0` 显示，`1` 隐藏，`2` 完全移除 |

<Tip>
套索框属于 UI 交互矩形，通常以“像素坐标系”表达；而元素点集（笔迹采样点）是 EMR 坐标系。
需要做几何对齐时，先明确坐标系，再参考 [`坐标系`](/plugin-base/coordinate-system) 做换算。
</Tip>
...
```ts wrap
import { PluginCommAPI, PluginNoteAPI } from 'rtn-supernote-plugin-core';
...
- `setLassoBoxState(2)` 表示完全移除套索状态：通常用于“操作结束”场景，移除后用户侧套索工具条会被关闭
