...
调用 [`insertGeometry`](/api-reference/supernote-plugin/plugin-comm-api/insert-geometry) 可插入一个几何元素：
...
<Tip>
几何类型字段 `type` 的取值与结构见 [`Geometry`](/api-reference/supernote-plugin/types/geometry)。点坐标为屏幕像素坐标系。
</Tip>

## 获取套索几何图形

当你用“套索工具”框选了页面上的几何元素，可以调用 [`getLassoGeometries`](/api-reference/supernote-plugin/plugin-comm-api/get-lasso-geometries)
读取被选中的几何图形列表。
...
```
返回的result是 [`Geometry`](/api-reference/supernote-plugin/types/geometry)列表。
<Warning>
...
调用 [`modifyLassoGeometry`](/api-reference/supernote-plugin/plugin-comm-api/modify-lasso-geometry) 完成修改：
...
- 几何数据结构：[`Geometry`](/api-reference/supernote-plugin/types/geometry)
- 插入几何接口：[`insertGeometry`](/api-reference/supernote-plugin/plugin-comm-api/insert-geometry)
- 获取套索几何接口：[`getLassoGeometries`](/api-reference/supernote-plugin/plugin-comm-api/get-lasso-geometries)
- 修改套索几何接口：[`modifyLassoGeometry`](/api-reference/supernote-plugin/plugin-comm-api/modify-lasso-geometry)
