...
| 创建套索选择 | [`lassoElements(rect)`](/zh/api-reference/supernote-plugin/plugin-comm-api/lasso-elements) | 通过矩形区域创建套索选择（像素坐标） |
| 调整套索框大小 | [`resizeLassoRect(rect)`](/zh/api-reference/supernote-plugin/plugin-comm-api/resize-lasso-rect) | 调整套索框矩形大小。当前仅支持等比例缩放 |
