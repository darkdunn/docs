...
调用 [`PluginNoteAPI.setLassoTitle`](/zh/api-reference/supernote-plugin/plugin-note-api/set-lasso-title) 可以把“套索选中的笔划或几何图形”设置为标题。
示例代码如下：
```ts wrap
...
调用 [`PluginNoteAPI.getLassoTitles`](/zh/api-reference/supernote-plugin/plugin-note-api/get-lasso-title)
接口获取套索的标题数据，样例代码如下：
...
```
[`PluginNoteAPI.getLassoTitles`](/zh/api-reference/supernote-plugin/plugin-note-api/get-lasso-title)
这个接口只能在存在套索上下文时调用。返回的是 [`APIResponse<Title[]>`](/zh/api-reference/supernote-plugin/types/api-response) 对象。
`result` 为 [`Title`](/zh/api-reference/supernote-plugin/types/title) 对象数组。

[`Title`](/zh/api-reference/supernote-plugin/types/title)的数据结构如下：
```ts wrap
class Title {
...
当套索选中标题之后，可以通过 [`PluginNoteAPI.modifyLassoTitle`](/zh/api-reference/supernote-plugin/plugin-note-api/modify-lasso-title)
修改套索的标题数据，样例代码如下：
```ts wrap
import { PluginNoteAPI } from 'sn-plugin-lib';
