---
title: "insertElements"
description: "插入元素数据到页面中。"
---

```ts
static insertElements(notePath: string, page: number, elements: Element[]): Promise<APIResponse<boolean>>;
```

**参数**

| 参数 | 类型 | 说明 |
| --- | --- | --- |
| `notePath` | `string` | 笔记路径 |
| `page` | `number` | 页面索引（从 1 开始） |
| `elements` | [`Element[]`](/api-reference/supernote-plugin/types/trail) | 元素数据数组 |

**返回**

- `APIResponse<boolean>`：`result === true` 表示插入成功

## 示例

```ts wrap
import { PluginFileAPI, PluginCommAPI, type Element } from 'rtn-supernote-plugin-core';

/**
 * 创建一个 Element 并插入到指定页面的示例。
 */
export async function exampleInsertElements() {
  const notePath = '/storage/emulated/0/Note/demo.note';
  const page = 1;

  const createRes = await PluginCommAPI.createElement(page, 0, 0);
  if (!createRes.success) {
    throw new Error(createRes.error?.message ?? 'createElement 调用失败');
  }

  const element = createRes.result as Element;
  const res = await PluginFileAPI.insertElements(notePath, page, [element]);
  if (!res.success) {
    throw new Error(res.error?.message ?? 'insertElements 调用失败');
  }
  return res.result;
}
```

