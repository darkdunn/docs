...
| `penUpListener` | `PluginEventListener` | 事件回调对象，实现 `onMsg(msg)`。<br />`msg` 数据结构与 `event` 类型对应。<br />当 `event` 为 `event_pen_up` 时，`msg` 为 [`Element`](/api-reference/supernote-plugin/types/trail)[] 元素数组 |
...
```ts wrap
import { PluginManager, type Element } from 'rtn-supernote-plugin-core';
