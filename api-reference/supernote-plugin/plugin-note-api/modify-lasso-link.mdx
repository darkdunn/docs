...
读取套索链接数据时可能会遇到 `linkType=6`（摘录链接）；该类型当前只能读取，无法通过本接口修改。相关结构以 [`Link`](/api-reference/supernote-plugin/types/link) / [`LassoLink`](/api-reference/supernote-plugin/types/lasso-link) 为准。
</Note>
