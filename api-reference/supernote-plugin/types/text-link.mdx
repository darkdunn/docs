...
| `rect` | [`Rect`](/api-reference/supernote-plugin/types/rect) | 文本区域矩形（像素坐标，需为非零面积） |
...
<Note>
`TextLink` 用于创建“文字链接”。如果你是从套索中读取链接数据，请参考 [`Link`](/api-reference/supernote-plugin/types/link) 与 [`LassoLink`](/api-reference/supernote-plugin/types/lasso-link)。
</Note>
