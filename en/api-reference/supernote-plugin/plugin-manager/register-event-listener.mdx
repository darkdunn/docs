...
| `penUpListener` | `PluginEventListener` | Event callback object that implements `onMsg(msg)`.<br />The `msg` structure depends on the `event` type.<br />When `event` is `event_pen_up`, `msg` is an array of [`Element`](/api-reference/supernote-plugin/types/trail). |
