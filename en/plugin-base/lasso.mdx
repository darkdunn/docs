...
Lasso is a common selection interaction in NOTEs/DOCs. After the user draws a region on the page, the system computes the selected element set and shows the lasso toolbar.
...
The selected element list can be obtained via [`PluginCommAPI.getLassoElements()`](/api-reference/supernote-plugin/plugin-comm-api/get-lasso-data). The return value is [`Element[]`](/api-reference/supernote-plugin/types/trail).

- Use `Element.type` to distinguish element categories (constants: [`ElementType`](/api-reference/supernote-plugin/types/trail#elementtype))
- For large point datasets (e.g., angle points, contour points), `Element` fields are provided as accessors ([`ElementDataAccessor`](/api-reference/supernote-plugin/types/trail-data-accessor)) to avoid transferring large point sets to RN at once

## Common APIs

| Goal | API | Description |
| --- | --- | --- |
| Get lasso rectangle | [`getLassoRect()`](/api-reference/supernote-plugin/plugin-comm-api/get-lasso-rect) | Returns lasso rectangle (`Rect`) |
| Update lasso rectangle | [`updateLassoRect(rect)`](/api-reference/supernote-plugin/plugin-comm-api/update-lasso-rect) | Updates the lasso rectangle. Currently only proportional scaling is supported |
| Get selected elements | [`getLassoElements()`](/api-reference/supernote-plugin/plugin-comm-api/get-lasso-data) | Returns `Element[]` (accessor fields are filled on success) |
| Control lasso box | [`setLassoBoxState(state)`](/api-reference/supernote-plugin/plugin-comm-api/set-lasso-box-state) | `0` show, `1` hide, `2` remove completely |
...
If you need to align them, confirm the coordinate systems first and convert as needed. See [Coordinate System](/plugin-base/coordinate-system).
</Tip>
