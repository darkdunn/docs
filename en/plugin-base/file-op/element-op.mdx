...
`Element` is the unified data structure for “all visible elements” in NOTE/DOC: strokes, titles, links, text boxes, geometric shapes, five-star elements, etc. are all represented as `Element`, and distinguished by `type`.

- Element types and fields: [`Element`](/api-reference/supernote-plugin/types/trail)
- Type constants: [`ElementType`](/api-reference/supernote-plugin/types/trail#elementtype)

This chapter provides a reusable workflow around the following capabilities:

- Get page elements: [`PluginFileAPI.getElements`](/api-reference/supernote-plugin/plugin-file-api/get-page-trails)
- Create a new element object: [`PluginCommAPI.createElement`](/api-reference/supernote-plugin/plugin-comm-api/create-trail)
- Insert element：[`PluginFileAPI.insertElements`](/api-reference/supernote-plugin/plugin-file-api/insert-trails)
- Modify element：[`PluginFileAPI.modifyElements`](/api-reference/supernote-plugin/plugin-file-api/modify-trails)
- Replace all elements on a page: [`PluginFileAPI.replaceElements`](/api-reference/supernote-plugin/plugin-file-api/replace-trails)
...
- On the RN side you receive an [`ElementDataAccessor`](/api-reference/supernote-plugin/types/trail-data-accessor) (a reference/handle)
...
<Tip>
After `getElements` succeeds, the SDK automatically normalizes element structures and fills in accessors. You can directly read typed fields such as `stroke/title/link/textBox/geometry` based on `type`.
</Tip>
